...
function processSum(numberList, callback) {
  const sumList = numberList.reduce(function(elements, sum) {
    return elements + sum;
  }, 0);
  return callback(sumList);
}
...
function getFullNames(runners) {
  ...
  runners.forEach(runner => {
    const name = runner.last_name + ", " + runner.first_name;
    Names.push(name);
  });
  return Names;
}
...
function firstNamesAllCaps(runners) {
  const Names = [];
  runners.forEach(runner => {
    const name = runner.first_name.toUpperCase();
    Names.push(name);
  });
  return Names;
}
...
//   return runners.reduce((sum, donation, index) => {
//     sum + runners[index].donation;
//   });
// }
